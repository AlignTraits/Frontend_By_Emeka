...
export default function CheckEligibility() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (agreed) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...data,
          schoolLocation: responseObj?.university?.region,
        }),
      );
      setIsLoading(true);
      try {
  ...
  const handleEligibilityCheck = async () => {
    ...
    for (let i = 0; i < maxEntries; i++) {
      if (i < newList.length) {
        ...
        temPayloadTwo[`ExamType${i + 1}Subjects`] = elem.subjects.map(
          (sub) => sub.subject,
        );
        temPayloadTwo[`ExamType${i + 1}SubGrades`] = elem.subjects.map(
          (sub) => sub.grade,
        );
      } else {
        // Pad with nulls
    ...
    try {
      setIsLoading(true);
      let now = new Date();
      const expiredate = responseObjTwo?.data?.payment_plan_expires_at
        ? new Date(responseObjTwo?.data?.payment_plan_expires_at)
        : now;
      ...
      if (
        responseObjTwo?.data?.subscriptionPlanStatus &&
        expiredate > now &&
        responseObjTwo?.ok
      ) {
        ...
        if (token && token.length > 0) {
          ...
          toast.success(
            "Please login and check eligibility results in your dashboard",
          );
          setTimeout(() => {
            navigate("/login");
          }, 1000);
  ...
  return (
    <div
      className={`relative ${displayRequirements ? "size-max" : "h-screen"} w-full bg-gradient-to-br from-[#CCE0F5] via-[#e9eff7] to-white`}
    >
      <Header />
