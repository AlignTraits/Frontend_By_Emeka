  ...
  return (
    <div className="w-full max-w-2xl mx-auto mt-[50px] bg-white border-[1px] border-[#ccc] shadow-md rounded-xl p-6">
      {showAddCourse ? (
        ...
        <>
          ...
          {!user?.careerResults && (
            <>
              <p className="mb-4 text-[#475467] text-sm">
                This is where it begins! Take a quick step to discover the best
                career pathways tailored to your strengths, interests, and
                goals.
              </p>

              <p className="mb-4 text-[#475467] text-sm">
                👉 Start your personalized career recommendation now and see
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            {user?.careerResults?.recommendedCareers &&
              user.careerResults.recommendedCareers.map(
      ...
      {showFirstTimeUser && (
        <div
          className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"
          onClick={() => setShowFirstTimeUser(false)}
        >
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-[90%]">
            ...
            <p className="mb-4">
              Find the career path that’s truly right for you. Take our quick
              assessment to get your personalized career recommendation.
            </p>
            <button
              onClick={() => {
